...
This archive records the first mock-only mobile PWA interface for GuLiCode.
The work intentionally stayed in the existing SolidJS/Vite app and did not
build or call a server-side API.
...
- Added a mobile-first interface for project selection, run detail, event
  stream, tool cards, run list, create-run form, bottom actions, and reports.
- Integrated `vite-plugin-pwa` using `generateSW` and `autoUpdate`.
- Configured runtime-sensitive paths as `NetworkOnly`:
  `/auth/*`, `/api/*`, `/runs/*`, and `/stream`.
- Replaced placeholder public favicon/PWA assets with real GuLiCode icon
  assets.
...
Workspace install passed from `GuLiCode` after setting Bun's linker to
`hoisted` in `bunfig.toml`:
...
1. Continue PWA frontend development by replacing mock state with a typed
   Collaboration Server client.
2. Build the Python service-side Collaboration Server boundary before exposing
   real run/project data to the mobile PWA.
3. Preserve the security boundary: mobile PWA never receives runtime tokens,
   workspace tokens, MCP bearer tokens, service tokens, private checkout paths,
   or local Codex homes.
