...
2. 将 `README.md`、`GUIDE_FOR_AGENTS.md`、`examples/HOWTO.txt` 明确纳入归档对照范围。
...
3. 强调“只更新 Cursor skill”时仅维护本 skill 文档，不改 `.codex` 副本。
...
3. 新增 `CLIWorkerBackend.create_from_registry()` 与 `_inject_skills()` 自动 skill 注入能力。
...
2. 新增 `init_skill_list.py`，合并 `.codex/skills` 与 `.cursor/skills` 到 `skill_list/`。
...
### 2026-04-16 — CLIWorkerBackend API、Demo 落地与框架定调

#### 摘要
1. 新增 `cluster.py` 中的 `WorkerConfig` 与 `CLIWorkerBackend` 门面。
2. 新增 `cluster start`、`run-parallel`、`run-chain` CLI。
3. demo 重写为基于 `CLIWorkerBackend`。
...
2. 落地初版 broker / agent / codex-worker。
...
1. **Adapter 边界**
   - 新增 `adapters.py`。
   - 落地 `CLIAdapter`、`CodexAdapter`、`AgentMessage`、`AdapterResult`。
   - `CodexAdapter` 继续兼容现有 `codex run` per-message 执行方式。
...
`multi_agent_tcp` 的核心仍然是多 agent 编排框架：通过 broker、registry、dispatch 与 cluster API，让多个 Codex CLI worker 在同一框架下并行、串行或归约执行任务。
...
- `CLIWorkerBackend.create_from_registry()`：推荐入口，按 registry 创建 worker，并自动注入 skill
