...
- CLI/model: `cli_kind` supports `codex` and `codex`; command strings are
  generated by the framework (`codex` or `codex`) and not shown as an input;
...
- `adapter_options`: advanced JSON passed through to the selected CLI adapter
  ...
  Codex base_args. Most users should leave it empty
