...
describe("blueprint runtime source", () => {
  test("starts runtime directly from selected start nodes after saving the project document", async () => {
    const source = await Bun.file(new URL("./blueprint-side-panel.tsx", import.meta.url)).text()
    ...
    expect(source).toContain("createBlueprintStartPlan(startDraft, { startNodes })")
