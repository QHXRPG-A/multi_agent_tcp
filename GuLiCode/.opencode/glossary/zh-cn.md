...
- `GuLiCode` (preserve casing in prose; keep `opencode` only when it is part of commands, package names, paths, or code)
- `GuLiCode Zen`
- `GuLiCode CLI`
- `CLI`, `TUI`, `MCP`, `OAuth`
...
- Avoid `opencode` in prose when referring to the product name; use `GuLiCode`
